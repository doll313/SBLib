...
@implementation NSData (EMSB_JSON)

- (id)sb_objectFromJSONData {
    id result = nil;
//    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 5.0f) {
        result = [NSJSONSerialization JSONObjectWithData:self options:0 error:nil];
//    }else{
//        result = [self objectFromJSONData];
//    }
//    
    if (result == nil) {
        //可能后台JSON有问题，尝试修复
        NSString *originalString = [[NSString alloc]initWithData:self encoding:NSUTF8StringEncoding];
        result = [originalString sb_objectFromJSONString];
    }
...
- (id)sb_objectFromJSONDataUsingEncoding:(NSStringEncoding)encoding {
    NSString *jsonString = [[NSString alloc]initWithData:self encoding:encoding];
    NSData *transformedData = [jsonString dataUsingEncoding:NSUTF8StringEncoding];
    
    return [transformedData sb_objectFromJSONData];
}
